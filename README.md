...
- **Live data** — fetches the top 1500 stations by popularity from
...
- **3D vector globe** — powered by [globe.gl](https://github.com/vasturiano/globe.gl).
  Stations are simple colored dots; names and details appear only in hover tooltips.
  Clicking a dot starts playback and highlights the matching card in the list (and vice
  versa). The base map is bundled, so the globe works offline.
...
- Country landmasses are drawn as subtle dotted hex-polygons from bundled
